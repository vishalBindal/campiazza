...
def main(request):
    items = Item.objects.all().order_by('-created_datetime')
    count = 0
...
def profile_view(request, id):
    ...
    return render(request, 'User/profile_page.html',
                  {'profile': profile,
                   'user_view': user,
                   'request': request,
                   'items': Item.objects.filter(buyer_id=request.user.id).order_by('-buy_time'),
                   'seller_items': Item.objects.filter(seller=user).order_by('-created_datetime')
                   })
...
def search(request):
    ...
    items = Item.objects.filter(Q(name__icontains=query) | Q(description__icontains=query)).order_by(
        '-created_datetime')
        ...
        if choice in query:
            if request.user.is_authenticated:
                available = Item.objects.filter(
                    Q(buyer_id=0) | Q(buyer_id=request.user.id) | Q(seller=request.user)).order_by('-created_datetime')
            else:
                available = Item.objects.filter(Q(buyer_id=0)).order_by('-created_datetime')
